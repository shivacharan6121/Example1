...
def download_data():
    try:
        ...
        cursor.execute("SELECT * FROM parts")
        rows = cursor.fetchall()
        with open("connectors_data.csv", "w", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["Part No", "Quantity"])
            writer.writerows(rows)
        conn.close()
        CustomMessageBox("Success", "Data downloaded successfully in your system\n in CSV format: connectors_data.csv!", "#228B22")
    except pymysql.Error as err:
        CustomMessageBox("Error", f"Database Error: {err}", "#8B0000")
